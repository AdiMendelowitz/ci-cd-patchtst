...
\begin{document}
    ...
    \begin{abstract}
        ...
        the absolute CD$-$CI difference stays inside a pre-specified $1\%$ relative-MSE band in both directions. What does not
    ...
    No CD/CI significance test is reported, because the CD numbers do not exist. ECL is included as evidence of the scaling
    ...
    each isolating one confound, and then summarises the resulting effect sizes against a pre-specified threshold. We fix
    the practical-equivalence threshold at $1\%$ of the CI mean test MSE, pre-specified before these robustness controls
    were run; differences smaller than this in either direction are treated as practically irrelevant regardless of
    statistical significance. This is an operational cutoff on observed effect sizes, not a formal statistical-equivalence
    claim over the reported confidence intervals.
