...
\begin{document}
    ...
    \begin{abstract}
        ...
        minimum erases the gap (CD$-$CI of $-0.0031$ MSE, 95\% CI $[-0.0109, +0.0046]$). A closed-form analysis of the
        generative model shows this near-parity is analytically expected, not merely observed: at the evaluation horizon, no
        predictor, channel-independent or channel-dependent, has any cross-channel signal left to exploit. A cross-variate
        prediction head ties CI at every $\gamma$ and removes the coupling-dependent widening, locating that widening in the
        shared head rather than the encoder. Matching the gradient-update budget across modes removes the deficit, which rules
        out the per-epoch step-count asymmetry as its source, and a block-covariance family carries the same no-advantage finding
        beyond compound-symmetry correlation. Across every tested synthetic cell (AR(1), leader-follower, and block-covariance)
        the absolute CD$-$CI difference stays inside a pre-registered $1\%$ relative-MSE band in both directions. What does not
        equalise is cost: CD requires smaller feasible batches and far more gradient steps per epoch, its batch size collapses
        to one at $C=84$, and at $C=321$ (ECL) the flattened-token formulation does not finish under a single-T4 budget while CI
        trains stably. On the observational ETTh1 ($C=7$), a matched-budget comparison still gives CI a modest but reliable edge
        (pooled CD$-$CI of $+0.0147$ MSE, 95\% CI $[+0.0047, +0.0246]$, significant at horizons 96 and 720). On the constructed
        AR(1), leader-follower, and block-covariance processes, neither mode shows an accuracy gap that clears our $1\%$ bar once
        each is selected fairly, and CD stays substantially more expensive to train. CI is the better default on accuracy per
        unit compute. All findings are scoped to PatchTST's flattened-token CD formulation, the generative families studied, and
        a single-T4 training budget.
    ...
    12-hour wall-clock limit, as in a standard Kaggle notebook session. Kaggle's dual-T4 accelerator option was selected
    for these sessions, but all training and inference reported here remains single-device: no run uses more than one GPU
    through model or data parallelism, so the reported VRAM ceiling and wall-clock figures reflect one T4's actual
    ...
    trains CD successfully at $P = 4$ under the same single-device T4 budget, with full five-seed coverage across $\gamma$
    still in progress (Table~\ref{tab:boundary}).
    ...
    feasible and a dedicated boundary tranche is in progress; the $\gamma \in \{0.0, 0.3\}$ cells remain dashes pending
    that tranche, while $\gamma \in \{0.6, 0.9\}$ currently carry the CI-only figure from earlier runs. This joint sweep
    ...
    \begin{table}[H]
        ...
        At $P = 4$, CD is feasible and a dedicated tranche is in progress; cells show the CI-only figure or a dash pending
            completion, not infeasibility.}
        \label{tab:boundary}
        \begin{tabular}{crrrr}
            \toprule
    ...
    \begin{figure}[H]
        ...
        \caption{Leader-follower VAR(1): CD/CI test-MSE ratio across the $(P, \gamma)$ grid, mean over seeds
            ...
            run ($P \in \{8, 16\}$); grey cells mark where CD was not run: at $P = 2$, infeasible on the T4; at $P = 4$,
            feasible but not yet complete across all seeds and $\gamma$. Among the coloured cells, none
            favours CD, and the ratio stays within $0.15\%$ of unity throughout.}
    ...
    channels. Leader and isolate channels receive no incoming coupling and remain diagonal, so their bound is exact and
    matches the other families. For followers we report a sandwich instead: the CD-restricted bound is a hard lower limit
    on achievable MSE, and 1.0 (no signal exploited) is a hard upper limit on the CI-restricted ceiling, since
    conditioning on strictly more information can never increase the population-optimal forecast error. Table~\ref{tab:theoretical_bounds}
    reports this bound across the full $\gamma$-by-$h$ grid. At $h = 96$, the paper's evaluation horizon, the sandwich has
    closed to zero width, to float64 precision, for every tested $\gamma \in \{0.0, 0.3, 0.6, 0.9\}$. At $h = 1$ it has
    not, and the available signal grows with coupling strength: $\gamma = 0.9$ leaves as much as 97\% of baseline
    variance exploitable at that horizon.

    The empirical near-parity result reported throughout this paper is consistent with an analytical ceiling that leaves
    essentially nothing to exploit at the horizon actually evaluated, rather than only an observed absence of an effect.
    We make no claim about horizons this paper does not evaluate.

    \begin{table}[H]
        \centering
        \caption{Follower channel: theoretical lower bound on CD forecast MSE (z-scored units) across the leader-follower
        coupling sweep, derived analytically from the generative model. The true CI-restricted error lies in
            $[\text{bound}, 1.0]$; the upper bound of 1.0 is exact only once the interval has already collapsed. At
            $\text{pred\_len} = 96$, the paper's evaluation horizon, the interval is closed to float64 precision for
            every tested $\gamma$.}
        \label{tab:theoretical_bounds}
        \begin{tabular}{crrrr}
            \toprule
            $h$ & $\gamma = 0.0$ & $\gamma = 0.3$ & $\gamma = 0.6$ & $\gamma = 0.9$ \\
            \midrule
            1   & 0.3600         & 0.1283         & 0.0523         & 0.0272         \\
            5   & 0.8926         & 0.5906         & 0.4591         & 0.4083         \\
            10  & 0.9885         & 0.8947         & 0.8483         & 0.8294         \\
            50  & 1.0000         & 1.0000         & 1.0000         & 1.0000         \\
            96  & 1.0000         & 1.0000         & 1.0000         & 1.0000         \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    correlation. This null is not only empirical: the closed-form sandwich bound derived in Section~\ref{sec:theoretical_bound}
    has already collapsed to zero width at $h=96$ for every tested coupling strength, so the near-parity result is the
    one the generative model predicts, not merely the one the training runs happened to produce. No tested synthetic cell shows
    an absolute CD$-$CI difference outside a pre-registered
